...
def evaluate(args):
    ...
    if args.gradcam:
          ...
          cam = grad_cam.generate_cam(query_tensor, max_cell)  # Generate the cumulative CAM for this layer
          # Normalize CAM for visualization
          cam = cam - cam.min()  # Translate to have minimum at 0
          cam = cam / cam.max()  # Scale to have maximum at 1
    ...
    if args.layercam:
        grad_cam = modules.LayerCAM(model)  # Initialize GradCAM for the specific layer
        cam = grad_cam.generate_cam(query_tensor, max_cell)  
