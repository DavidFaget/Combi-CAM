...
class GradCAM:
    def __init__(self, model, num_layer):
        ...
        # Update to use the last convolutional block correctly and register full backward hook

        #final_layer = self.model.backbone.base_cnn._conv_head
    ...
    def generate_cam(self, input_image, class_idx):
        ...
        target_size = (input_image.size(2), input_image.size(3))  # Correctly accessing height and width
        cam = cam.unsqueeze(0)
        cam = cam.unsqueeze(0)
...
class GradCAMplusplus:
    def __init__(self, model):
        ...
        # Update to use the last convolutional block correctly and register full backward hook
        final_layer = self.model.backbone.base_cnn._blocks[31]  # Modify as per your model's architecture
...
class LayerCAM:
    ...
    def generate_cam(self, input_image, class_idx=None):
        ...
        # Element-wise multiplication of gradients and feature maps
        element_wise_product = gradients * feature_maps  # [C, H, W]

        # Summing across channels to compute the raw CAM
        cam = torch.sum(element_wise_product, dim=0)  # [H, W]
